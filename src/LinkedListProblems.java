...
public class LinkedListProblems {
	...
	public static void main(String[] args) {
		...
		while (t-- > 0) {
			...
			//LLPair p = LLUtils.reverseLL(head, tail);
			//LLUtils.printLL(p.head);
			n = in.nextInt();
			head1 = null;
			for (int i = 0; i < n; i++) {
				if (i == 0) {
			...
			//LLUtils.printLL(head1);
			//temp = LLUtils.mergeList(head, head1);
	...
	public static LLNode mergeList(LLNode h1, LLNode h2) {
		...
		if (h1 != null && h2 != null){
			if (h1.value <= h2.value) {
				lp.head = h1; lp.tail = h1; h1 = h1.next;
			} else {
				lp.head = h2; lp.tail = h2; h2 = h2.next;
			}
		}
